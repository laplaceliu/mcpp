...
HttpTransport::HttpTransport(int port)
...
void HttpTransport::on_message(MessageHandler handler) {
    message_handler_ = std::move(handler);
}

void HttpTransport::on_error(ErrorHandler handler) {
    error_handler_ = std::move(handler);
}

void HttpTransport::handle_http_request(const std::string& body) {
    if (message_handler_) {
        try {
...
void HttpTransport::send_sse_event(const std::string& event, const std::string& data) {
    std::string sse_data = HttpFramer::frame_sse(event, data);
...
std::string HttpFramer::frame(const std::string& message) {
...
std::string HttpFramer::frame_sse(const std::string& event, const std::string& data) {
    std::ostringstream oss;
    if (!event.empty()) {
        oss << "event: " << event << "\r\n";
    }
...
std::string HttpFramer::extract_body(const std::string& http_message) {
...
int HttpFramer::parse_content_length(const std::string& headers) {
